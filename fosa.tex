...
$$P_\theta(\theta\in \left[t_l(y),t_r(y)\right]|y) = \int_{t_l(y)}^{t_r(y)} f_{post}(\vartheta|y)d\vartheta = 1- \alpha$$
...
\noindent For regressions: $P(\text{covariate } x|y) = \sum_{k=1}^K \mathbbm{1}_{\{x_i \:\text{in}\: M_k\}}P(M_k|y)$
